...
function renderCard(p,widget=false,omitHeading=false){
  ...
  const heading=`<div class="card-heading"><div class="project-name-group"><button class="project-name" data-action="detail" data-key="${key}" title="${escape(p.name)}">${escape(p.name)}</button><span class="namespace">${escape(p.namespace)}${widget&&p.demo?'<span class="widget-demo">Демо</span>':''}</span></div>${!widget?repoButton(p):''}${widget?widgetControls:`<button class="icon-button ${p.pinned?'pinned':''}" data-action="pin" data-key="${key}" aria-label="${p.pinned?'Закрыть виджет':'Закрепить виджет'} ${escape(p.name)}" aria-pressed="${!!p.pinned}" title="${p.pinned?'Закрыть виджет':'Закрепить на рабочем столе'}">${icon('pin')}</button>`}</div>`;
...
function renderCollections(){
  ...
  if(selected)context.innerHTML=`<div><h2>${escape(selected.name)}</h2>${selected.error?`<p class="collection-error" role="status">Состав группы: ${escape(selected.error)}</p>`:''}</div><div class="context-actions">${selected.sources.length===1?`<button class="icon-button" data-action="external" data-url="${escape(selected.sources[0].webUrl)}" title="Открыть группу в GitLab" aria-label="Открыть группу в GitLab">${icon('external')}</button>`:''}<button class="icon-button" data-action="edit-group" data-key="${escape(selected.key)}" aria-label="Изменить группу ${escape(selected.name)}" title="Изменить группу">${icon('edit')}</button><button class="secondary-button ${selected.pinned?'pinned':''}" data-action="pin-group" data-key="${escape(selected.key)}" aria-pressed="${!!selected.pinned}" aria-label="${selected.pinned?'Закрыть виджет группы':'Закрепить группу'} ${escape(selected.name)}">${icon('pin')}${selected.pinned?'Закреплена':'Виджет группы'}</button></div>`;
  return selected;
}
...
function render(){
  ...
  if(widgetKey){
    ...
    if(g){
      ...
      updateWidget(`<header class="widget-bar"><div class="widget-title"><span title="${escape(g.name)}">${escape(g.name)}</span><small>${items.length}</small></div>${widgetTools(g.sources.length===1?{...g,webUrl:g.sources[0].webUrl}:null)}</header>${g.error?`<p class="collection-error" role="status">Состав группы: ${escape(g.error)}</p>`:''}<div class="group-body ${mode==='full'?'group-expanded':''}">${items.length?items.map(p=>mode==='compact'?compactRow(p,true):mode==='stages'?stageBlock(p):renderCard(p)).join(''):'<div class="empty-state"><h2>В группе пока нет проектов</h2></div>'}</div><footer class="app-footer"><div><span class="live-dot"></span><span>${g.demo?'Демо · пример группы':state.updating?'Обновление…':`Каждые ${state.interval/1000} сек.`}</span></div><span>${items.filter(p=>p.error||p.pipeline?.status==='failed').length} ошибок</span></footer>`);
